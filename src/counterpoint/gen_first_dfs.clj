(ns counterpoint.gen-first-dfs
  (:require [clojure.java.shell :as sh]
            [counterpoint.cantus-firmi-examples :refer [fux-a salieri-d]]
...
(defn generate-reverse-random-counterpoint-dfs [position key cantus]
  (try
    (let [rev-cantus (reverse cantus)
          ...
          ;; dfs-first (first (generate-dfs-solutions root-node candidates next-node solution?))
          ;; _ (println "DFS" dfs-first)
          ]

      (generate-dfs-solutions root-node candidates next-node solution?)
      ;;  (into [] (reverse (nth dfs-first 2)))
      )
    (catch Exception _
      (println "Trying again...")
      (generate-reverse-random-counterpoint-dfs position key cantus))))
