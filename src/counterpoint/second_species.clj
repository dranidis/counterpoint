(ns counterpoint.second-species
  (:require [counterpoint.core :refer [interval note->number]]
            ...
            [counterpoint.species-type :refer [dissonance-score get-cantus
                                               get-counter get-low-high
                                               get-position harmony-score make-species]]
...
(defn evaluate-second-species [species & {:keys [verbose]
  ...
  (let [[low high] (get-low-high species)
        ...
        score-diss (dissonance-score low high)
        ;; score 0
        melody-s (melody-score
                  (filter #(not= % :rest) (get-counter species))
                  :verbose verbose)
        total (float (/ (+ score score-diss melody-s) (count (get-cantus species))))]
    (when verbose
      (println "2nd species evalution")
      (println "-- HARM SCORE: " score)
      (println "-- DISS SCORE: " score-diss)
      (println "-- MEL SCORE: " melody-s))
    total
    ;; {:harm score :diss score-diss :mel melody-s :total total}
    ;
    ))
